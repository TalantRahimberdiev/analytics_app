  ...
  const uploadFile = (event) => {
    ...
    ExcelRenderer(fileObj, (err, resp) => {
      if (err) {
        ...
        const { rows } = resp;
        setExcelRaws(rows && rows.slice(1, rows.length).slice(0, 4));
      }
    });
  };
  const add = async () => {
    for (let i = 0; i < excelRaws.length; i++) {
      let bank_object = data.find((item) => item.title === excelRaws[i][0]);
      const task = {
        bank_id: bank_object && bank_object["id"],
        interest_rate: excelRaws[i][1],
        promo_campaign_name: excelRaws[i][2],
        url: excelRaws[i][3],
        entry_date: excelRaws[i][4],
      };
