  ...
  endpoints: (builder) => ({
    ...
    update_campaign: builder.mutation({
      query: ({ campaign_no, ...task }) => ({
        ...
        responseHandler: (response) =>
          response.status === 200
            ? alert(`Кампания обновлен успешно со статусом: ${response.status}`)
            : alert(
    ...
    add_action_manually: builder.mutation({
      query: (data) => ({
        url: `client_action/create_action_manually/`,
        method: 'POST',
        body: data,
        responseHandler: (response) =>
          response.status === 200
